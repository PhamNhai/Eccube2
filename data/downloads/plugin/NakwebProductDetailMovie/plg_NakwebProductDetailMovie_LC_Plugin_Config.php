 ...
 */
class plg_NakwebProductDetailMovie_LC_Plugin_Config extends LC_Page_Admin_Ex {
    ...
    function init() {
        ...
        // サブタイトル
        $this->tpl_subtitle = "動画埋め込みプラグイン ProductMovies!";
        // 管理画面で表示する説明文
        $this->tpl_note     = "商品詳細ページにYouTube(c)のストリーミング動画を埋め込みます。";
    ...
    function lfInitParam(&$objFormParam) {
        $objFormParam->addParam('表示動画数', 'how_movie', '', '', array('SPTAB_CHECK', 'NUM_CHECK'));
        $objFormParam->addParam('関連動画表示許可', 'other_movie_flg', '', '', array());
        $objFormParam->addParam('ループ再生許可', 'loop_flg', '', '', array());
        $objFormParam->addParam('全体画面再生許可', 'fullscreen_flg', '', '', array());
    }
